...
- 以首张卡启动成功的时间为基准设置固定检查点, 每隔 30 分钟执行一轮状态检查; 不得自行缩短周期. Agent 消息或用户输入可触发额外检查, 额外检查后继续等待原定检查点, 不重置或取消后续定时检查.
...
- 每轮状态检查只运行 `kanban check` 并读取全组卡片状态, 核对前置任务是否已进入 `done`.
- 主控 Agent 不检查执行进程, 不进入子任务会话, 不向执行 Agent 催促或发指令, 不中断, 恢复, 重启, 接管或改派子任务. 卡片状态异常, 顺序冲突或相邻两轮检查均无进展时, 只记录现状并向用户报告, 等用户决定交接, 改契约或终止; `working` 卡不得再次 `start`.
