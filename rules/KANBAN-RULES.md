...
- `tui` 默认并排显示活跃栏目, `--single` 每次只显示一个栏目, `--theme` 指定初始配色主题 (默认 auto 跟随终端). 方向键或 `hjkl` 切换栏目和任务, PgUp/PgDn 按页翻动任务列表, `/` 搜索, Enter 查看任务卡, `a` 切换存档栏目, `t` 循环切换 auto/light/dark 主题, `r` 刷新, `q` 退出; 搜索覆盖标题, 任务 ID, 任务组, 类型, 负责人和状态.
