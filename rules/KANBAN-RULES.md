...
- `notify` 是主控向原执行 Agent 派回事项的单一接口. 它与 `resume` 一样只接受 `review/` 或 `working/` 卡, 必须且只能给非空的 `--message` 或 `--message-file`, `--timeout` 为正的有限秒数且默认 120 秒. 命令先按 `窗口` 地址尝试直投并等待本次唯一 marker 回执: herdr 用 `pane get` 验证 pane 存在、Agent 与会话 UUID 完全匹配且状态为 `idle`, 属于身份级校验; tmux/tmux-session 验证 `pane_dead=0`, `pane_in_mode=0` 且 `pane_current_command` 与 Agent 进程名一致, 只是较弱的进程名级校验, 会被前台子进程遮蔽、可能误认同名进程且不能区分会话, 不得与 herdr 表述为等价. 正文写入仅当前用户可访问的 `0700` 临时目录及 `0600` 文件, 终端只收到一行以 `# onevoke-notify:` 开头、含绝对路径和 marker 的指令; herdr 用 `pane wait-output --source recent`, tmux 用有界 `capture-pane` 轮询确认 marker. foreground/console、缺地址的旧卡、探查或投递失败、回执超时都由命令内部自动恢复卡片记录的原会话并验证 Agent 没有秒退; 主控不另行调用 `resume`. 直投回执或恢复存活校验通过后才把 `review` 卡迁到 `working` 并输出实际通道; 两路都失败时非零退出并同时报告原因, 卡片正文与原状态不变.
