...
- 编排结束时汇总执行顺序, 并行情况和组级结果, 再按卡片列出"未处理问题", 分类与记录要求按 `~/.agents/REVIEW-RULES.md` 的未处理项清单; 没有写"无", 每项另写任务 ID. 成功时仍发送各卡完成报告; 终止时列未完成卡和终止决策.
...
- 未处理问题的分类与记录要求按 `~/.agents/REVIEW-RULES.md` 的未处理项清单, 另加验证缺口和后续任务; 同一根因只计一次. 安全审核超时项附发送时间和超时时间.
