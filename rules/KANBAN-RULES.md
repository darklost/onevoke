...
- 主控决定审核时机与批次: 把已进 `review/` 的卡按同一模块, 同一里程碑或依赖链分批, 一批的 diff 应能被 reviewer 精读; 不强制全组一次, 也不强制每卡一次. 批次的 CWD, base, task context 和角色流转按 `REVIEW-RULES.md`「任务组的组级审核」执行. 同一 base 下多批之间结论沿用, 已通过角色只对新批次增量复审.
