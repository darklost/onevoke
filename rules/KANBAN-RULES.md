...
- 每张就绪的 `todo` 卡都用 `kanban start <task-id>` 启动, launcher 默认取 Onevoke 配置, `--launcher` 只覆盖本次; 一个任务组内只用同一种 launcher, tmux 不可用时报告阻塞. 首轮启动无前置任务的卡, 之后只启动全部前置卡都在 `done/` 的卡; 同时就绪且无资源冲突的卡并行启动, 禁越过依赖提前启动.
