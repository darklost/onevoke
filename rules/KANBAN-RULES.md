...
- 一个目标需要多名负责人, 多条可独立交付链路或明显超过单卡可控范围时, 建成任务组并拆成多张任务卡. 拆分优先减少卡片间依赖, 让更多卡片能并行执行; 不为追求并行而制造重叠改动或虚假边界.
- 任务组不是新的入口类型或状态目录. 组内每张卡仍独立领取, 验收和迁移. 每张卡都在 `讨论与决策` 记录相同的 `任务组: <标识>`, 并用 `前置任务: <任务 ID>` 列出必须先进入 `done/` 的组内卡片; 多项用逗号分隔, 没有则写 `前置任务: N/A`.
- 目录内链接用相对路径, 入口搬家后还能用.
...
<按时间追加关键结论, 不写无意义流水. 任务组卡片在此记录任务组和前置任务>
...
- 一个总体目标包含多项能独立领取和验收的工作 → 任务组, 拆成多张小任务或大任务卡; 不建一张包办全部实现的总卡.
...
- 任务组卡片的 `任务组` 和 `前置任务` 在进 `todo/` 前填定. 单卡任务不用写这两项.
...
- 选 1 即同时确认计划, 开发和看板流程, 不再二次确认. 单卡任务立刻按本文件跑三步:
...
- 任务组则一次性创建并填完全部卡片, 写清共同的 `任务组` 和各自的 `前置任务`. 用户确认的是整个任务组计划; 编排 Agent 可据此将全部卡片移进 `todo/`, 再按「任务组编排」启动就绪卡片, 不逐卡向用户重复确认.
...
- 创建任务组时先列出完整卡片清单和依赖关系, 检查不存在循环依赖, 缺失前置卡片, 重叠职责或无法独立验收的卡片. 能同时开始的卡片应没有彼此前置关系.
...
- 启动者在任务完成或明确交接前持续负责协调. tmux 模式下定期查看卡片状态和执行 Agent 是否仍在运行; foreground 模式等待 Agent 退出后检查任务结果. 发现退出, 阻塞或状态长期未更新时按异常恢复规则处理, 不把 `start` 成功当成协调职责结束.
- 同文件系统里的移动就是领取原语. 初始方案不加 lock 服务, 守护进程, 数据库, ID 分配器.
- 领了之后只有负责人能改入口内文档. 别的 Agent 能读, 不许并发写或移动.

## 任务组编排

- 用户要求启动任务组后, 负责启动组内任务卡的 Agent 立即转为编排 Agent. 编排 Agent 不领取实现卡, 不并发修改卡片正文或任务代码; 它负责依赖排序, 启动, 巡检, 恢复协调和总体完成确认.
- 编排 Agent 启动前读取全组卡片, 核对 `任务组` 一致, `前置任务` 都指向组内现有卡片, 依赖图无环, 每张卡契约完整, 修改范围可隔离. 有缺口就先报告, 不启动不完整或顺序不明的任务组.
- 编排 Agent 将已确认的全组卡片移入 `todo/`. 首轮启动所有 `前置任务: N/A` 的卡片; 之后只有一张卡的全部前置卡片都在 `done/` 时, 才能启动该卡. 同时就绪的卡片尽量并行启动, 但存在 worktree、文件所有权、集成顺序或资源冲突时按依赖顺序串行.
- 编排 Agent 必须保持编排会话运行, 使用当前环境可用的定时等待或监控机制, 每隔 30 分钟执行一次巡检. 不用守护进程替代 Agent, 不把高频轮询写进 `kanban` CLI. 两次定时巡检之间遇到任务完成, Agent 消息或用户输入时可立即处理, 不必等满 30 分钟.
- 每轮巡检至少运行 `kanban check` 并查看全组卡片状态, 再检查 `working/` 卡片对应 Agent 或 launcher 是否仍在运行、卡片记录的阻塞和完成报告. 不以进程存活代替任务状态, 不因暂时没有新提交就判定失败.
- 已完成卡片按各自规则验收和迁入 `done/`; 它的完成会解锁后继卡片. 编排 Agent 每轮都启动新就绪卡片, 确保依赖顺序正确, 不让已就绪卡片无故等待下一轮.
- 任务 Agent 退出但卡片未完成, 启动失败, 长期无进展或报告阻塞时, 编排 Agent 先核实卡片和 worktree 现场. 可在原任务范围内恢复同一 Agent 会话或重新启动执行 Agent; 涉及换负责人, 改契约, 放弃卡片, 接受风险或无法隔离的冲突时停止受影响链路并请用户决策, 其他独立链路继续推进.
- 编排 Agent 直到全组卡片都进入 `done/` 才结束编排. 任一卡仍在 `backlog/`, `todo/`, `working/`, 被归档或移入 `trash/`, 都不算任务组完成; 后两种情况还必须取得用户对总体目标变更的明确确认.
- 全组完成后, 编排 Agent 汇总各卡完成报告, 说明执行顺序, 并行情况, 组级验收结果和遗留项. 另设“未处理问题”列表, 按任务卡逐项汇总全组所有审核未处理项、验证缺口、接受的风险和遗留问题; 没有则明确写“无”. 每项至少写任务 ID、来源角色或类别、档位、问题、实际影响和未处理理由, 其中因用户 15 分钟无决策而继续的安全 finding 标明“超时忽略”. 单卡执行 Agent 的完成报告仍照常发送, 不由组级汇总替代.
...
- `CSA` 或 `Hacker` finding 需要用户决策时, 按 `~/.agents/REVIEW-RULES.md` 发送完整选项并计时. 15 分钟无决策就把该 finding 以“超时忽略”写入小任务的 `实施与验证` 或大任务的 `plan.md`, 然后继续 QA 和后续流程; 这不是审核 `PASS` 或用户接受风险.
...
- 审核未处理项逐项写角色, 档位, 问题, 实际影响, 未处理理由. 因 `CSA` 或 `Hacker` finding 等待 15 分钟仍无用户决策而继续时, 必须写明“超时忽略”、发送时间和超时时间, 不得写成 `PASS`、用户确认或接受的风险. 剩余问题数统计审核未处理项, 验证缺口, 接受的风险, 遗留问题, 后续任务; 同一根因只计一次, 已修问题和单纯计划偏差不计.
