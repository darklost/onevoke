...
- `start` 的 Agent 默认取 Onevoke 配置 `kanban_agent`, welcome 未完成时回落到 Codex; `--agent` 只覆盖本次. 大任务用 Codex `gpt-5.6-sol/high` 或 Claude `opus/high`, 小任务用对应 `medium`; Grok 使用 CLI 默认模型和推理强度.
...
- 启动者协调至任务完成或明确交接: tmux 模式定期查看卡片和进程, foreground 模式在 Agent 退出后检查结果. `start` 成功不解除该责任.
...
- 卡片进入 `done/` 后, 执行 Agent 按模板汇报一次; 未进 `done/` 或代码未集成时不发. 卡片先记录同一批事实; 报告字段不得省略, 无内容写 `无` 或 `N/A`.
- 只写实际验证结果, 不把失败, 未执行或环境阻塞写成通过; 最终提交用完整 40 位 SHA.
