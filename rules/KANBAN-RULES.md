...
- `start` 未传 `--agent` 时读取 Onevoke 配置的 `kanban_agent`, 配置不存在或 welcome 未完成时回落到 Codex; `--agent codex|claude|grok` 可覆盖本次选择. 指定任务只收 `todo` 卡片; 不指定就列 `todo` 让用户按编号选.
...
- `start` 默认按 Onevoke 配置的 launcher 启动; `--launcher` 覆盖本次启动, 项目规则需要固定模式时明确要求 Agent 传该参数, 不改整机配置. `tmux` 模式只在当前 session 新建并切 window, 不建 session; window cwd 是项目根, 名 `kb-<任务标题>`. `foreground` 模式要求 stdin/stdout/stderr 都是 tty, 在当前终端前台运行并等待 Agent 退出.
