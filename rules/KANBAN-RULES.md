...
- 任务组不是新的入口类型或状态目录. 组内每张卡仍独立领取, 验收和迁移, 用相同的 `任务组` 标识关联; `前置任务` 填必须先进入 `done/` 的组内任务 ID, 多项用逗号分隔, 没有则写 `N/A`.
- 目录内链接用相对路径, 入口搬家后还能用.
...
- `任务组` 和 `前置任务` 在进 `todo/` 前填定. 单卡任务两项都写 `N/A`; 任务组内无依赖的卡片只把 `前置任务` 写 `N/A`.
