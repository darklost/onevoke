...
- 选择小任务或大任务前, 先判断总体目标能否拆成可独立领取, 验收或终止, 且资源不冲突的并行子任务. 能拆成并行子任务时必须建任务组, 不得仅因总体范围大而保留为单张大任务卡.
- 小任务是单文件卡片; 大任务是目标, 负责人, 验收和生命周期必须保持统一, 不能安全拆成并行交付, 且需要独立 spec, 按需分阶段计划和完整报告的单张卡片. 行数不是判据.
- 总体目标含多名负责人, 多条独立交付链路或多个可并行子任务时建任务组, 不建总实现卡. 组内每张子卡再按自身复杂度选择小任务或大任务形态.
- 拆卡应减少依赖以便并行, 且不得职责重叠; 不能安全隔离的同资源修改须建立依赖并串行, 但不影响其他无冲突子任务并行.
...
- 非任务组单卡以 tmux 启动后, 执行 Agent 在独立 window 直接向用户汇报, 启动者不巡检该卡: 启动成功后立即明确告知用户本会话不跟踪该任务进度, 当前 session 可以结束, 后续讨论下一个任务时另开会话. 用户明确要求跟踪时改按上一条协调.
- 同文件系统上的入口迁移是领取原语, 不增加 lock 服务, 数据库或 ID 分配器.

## 任务组编排

- 用户启动任务组后, 启动者成为主控 (编排) Agent, 只负责校验依赖, 按顺序启动就绪卡, 定时检查卡片状态和给出组级结论. 主控 Agent 不实现组内任务, 不检查或修改子任务的代码, worktree, commit, 测试或审核内容.
...
- 主控会话持续到任务组成功或用户明确终止. 首张卡启动成功后立即明确告知用户: 本会话是任务组主控, 须保留到全组按依赖顺序执行完毕, 不要结束当前 session; 提前结束会失去依赖校验, 顺序启动和组级结论.
- 首张卡启动后每隔 30 分钟执行一轮状态检查; Agent 消息或用户输入可触发额外的状态检查, 但不取消下一次定时检查.
- 每轮只运行 `kanban check` 并读取全组卡片状态, 核对前置任务是否已进入 `done`. 只有全部前置卡均为 `done` 时才能按已确认顺序调用 `kanban start --launcher tmux <task-id>` 启动新解锁卡片; 禁越过依赖提前启动.
- 主控 Agent 不检查执行进程, 不进入子任务会话, 不向执行 Agent 催促或发指令, 不中断, 恢复, 重启, 接管或改派子任务. 卡片状态异常, 顺序冲突或相邻两轮检查均无进展时, 只记录现状并向用户报告, 等用户决定交接, 改契约或终止; `working` 卡不得再次调用 `kanban start`.
- 全部组内卡进入 `done/` 才算成功. 任一卡进入 `archived/` 或 `trash/` 时, 须等待用户修改组契约或终止整组.
- 编排结束时汇总执行顺序, 并行情况和组级结果, 再按卡片列出"未处理问题", 分类与记录要求按 `~/.agents/REVIEW-RULES.md`「结论与故障处置」的未处理项清单, 另加验证缺口和后续任务; 没有写"无", 每项另写任务 ID. 成功时仍发送各卡完成报告; 终止时列未完成卡和终止决策.
...
- 合回时机取 `~/.agents/ONEVOKE-AGENTS.md`「看板任务完成」. 默认在验证和必要审核通过后按 `~/.agents/GIT-RULES.md`「集成与清理」直接 fast-forward 合回目标分支, 不等用户验收.
- 下列情况不集成, 也不迁 `done/`: 用户要求暂停或不合回, 必要审核未通过, 或集成, 清理失败. 卡片留 `working/`, 保留分支与 worktree, 记录阻塞及解除条件并报告用户.
- 用户在完成报告后测试发现的问题按新任务处理: 另建卡片并在 `讨论与决策` 指向原卡, 不把已进 `done/` 的卡退回 `working/`, 也不复用原卡继续改.
- 用户明确要求的验收或集成确认不适用 15 分钟超时.
...
- 未处理问题的分类与记录要求按 `~/.agents/REVIEW-RULES.md`「结论与故障处置」的未处理项清单, 另加验证缺口和后续任务; 同一根因只计一次. 安全审核超时项附发送时间和超时时间.
