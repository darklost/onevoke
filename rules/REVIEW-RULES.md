...
- 仅看板任务使用安全 finding 超时. 每项独立计时: 完整信息和选项送达用户时记录发送时间; 15 分钟内没有针对该项的明确决策, 就标记 "超时忽略", 记录角色、档位、问题、影响、发送时间、超时时间和理由, 然后继续. 多项一起发送时共用发送时间, 用户只回答部分项不停止其余项计时.
- "超时忽略" 不等于 `PASS`、用户确认或接受风险. 用户在卡片完成前补充决策时按最新指令处理. 非看板任务必须等待用户决策; `PM`、`QA` finding, 无法核实项, 契约或负责人变更, 验收、集成及其他确认也不得超时跳过.
