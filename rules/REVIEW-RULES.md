...
- 第二阶段提交决策时记录发送时间. 从完整问题、影响、修复方法和选项送达用户起满 15 分钟仍无决策, 将该 finding 标记为“超时忽略”, 记录角色、档位、问题、实际影响、发送时间、超时时间和未处理理由, 然后继续后续审核与任务流程. 不把超时写成 `PASS`、用户确认或用户接受风险; 用户在任务完成前补充决策时, 按最新明确指令处理.
- 15 分钟规则只适用于 `CSA` 或 `Hacker` finding 引发的第二阶段决策. `PM`、`QA` 的必修 finding, 主代理无法核实的问题, 任务契约变更, 负责人变更, 验收、集成和其他规则要求的用户确认均不得据此超时跳过.
...
- 一次有效审核要求 `PM` 与 `QA` 均无经主代理认可的 blocking、high、medium finding, 且按触发条件实际运行的 `CSA`、`Hacker` 基于同一 commit 返回结论; 安全角色无合格 finding 直接通过, 有合格 finding 须由用户完成决策或按第二阶段 15 分钟规则记为超时忽略.
- 被判「无法核实」的 blocking、high、medium 视同未通过: 该阶段不得放行, 按「主代理的核实义务」交用户决策, 用户判定为不必处理后才继续.
- 审核闭环结束时 (通过或中途终止), 必须向用户展示本轮全部未处理项, 一项都不省: 未修的 `low`、`推荐`、`建议`, 被主代理判定不成立或无法核实的 finding, 用户已确认接受的风险, 因 15 分钟无决策而超时忽略的安全 finding, 因后端故障未完成的角色, 未提供 `NON-BLOCKING` 段的角色. 每项写明来源角色、档位、问题内容、实际影响、未处理的理由. 禁只报"审核通过"或"无阻塞问题"就结束. 同一批内容写进任务卡和交付说明, 禁静默丢弃. 走豁免路径时本条不适用, 按豁免规则告知用户即可.
...
- 第二阶段未获用户决策且尚未满 15 分钟时, 保留分支和 worktree, 等待用户决定. 满 15 分钟后按本文件标记超时忽略并继续, 不再因此停止集成. 禁绕过 wrapper 直调 reviewer CLI.
