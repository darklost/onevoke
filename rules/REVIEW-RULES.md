...
- 一次完整审核中所有角色必须使用相同 `CWD`, base 与 task context, 同时触发的 `CSA` 与 `Hacker` 必须基于同一 commit. 各角色结论在同一 base 下沿用: 某角色通过后, 其他角色引发的后续修复不重启它; 只有安全角色的修复提交例外, 由 `QA` (改变功能行为时含 `PM`) 增量复审. 因此各角色通过的 commit 允许不同, 但最终 commit 必须是每个通过 commit 的后代. 非集成原因的 base 变更 (用户要求换 base, 分支重建, 或审核期被迫 rebase) 使全部结论失效, 从第一阶段重启; 集成流程内因 `develop` 前进而做的 rebase 不属此列, 按 `GIT-RULES.md`「集成与清理」的一次性门沿用已通过结论. task context 是权威需求契约: 短任务传单个字符串, 长任务传可读的绝对 spec 路径.
