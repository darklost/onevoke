...
[[ "$GROK_REVIEW_HOME" == /* ]] ||
  fail "GROK_REVIEW_HOME must be an absolute path: $GROK_REVIEW_HOME"
[[ "$CHECK_INTERVAL_SECONDS" =~ ^[1-9][0-9]*$ ]] ||
  fail "GROK_REVIEW_CHECK_INTERVAL_SECONDS must be a positive integer"
[[ "$MAX_RUNTIME_SECONDS" =~ ^[1-9][0-9]*$ ]] ||
  fail "GROK_REVIEW_MAX_RUNTIME_SECONDS must be a positive integer"
...
[[ -d "$GROK_REVIEW_HOME" && -r "$GROK_REVIEW_HOME" && -w "$GROK_REVIEW_HOME" ]] ||
  fail "Grok review home is not readable and writable: $GROK_REVIEW_HOME"
GROK_STATE_ROOT=$(cd "$GROK_REVIEW_HOME" && pwd -P)
readonly GROK_STATE_ROOT
if paths_overlap "$ROOT" "$TMP_ROOT" || paths_overlap "$ROOT" "$GROK_STATE_ROOT"; then
  fail "worktree overlaps a Grok-writable directory: $ROOT"
fi
...
command -v "$GROK_BIN" >/dev/null 2>&1 || fail "Grok CLI is unavailable: $GROK_BIN" 127

RUNTIME_DIR=$(mktemp -d "${TMPDIR:-/tmp}/grok-review.XXXXXX")
...
cleanup() {
  ...
  if ((REVIEW_STARTED)) && ! target_is_unchanged; then
    echo "Error: Grok review modified the target worktree: $ROOT" >&2
    exit_code=2
  fi
  rm -rf -- "$RUNTIME_DIR"
  exit "$exit_code"
...
generated output when relevant. Use only read_file, grep, and list_dir to inspect code. Do not
modify files, the index, refs, or the worktree. Begin the report with Role, Commit, Task Context,
and Reviewed Scope.
...
while kill -0 "$REVIEW_PID" 2>/dev/null; do
  ...
  if ((elapsed >= MAX_RUNTIME_SECONDS)); then
    echo "Error: Grok review exceeded ${MAX_RUNTIME_SECONDS} seconds" >&2
    stop_review
    cat "$ERROR_FILE" >&2
    cat "$OUTPUT_FILE"
    exit 124
  ...
  if ((SECONDS >= next_check)); then
    echo "Grok review is still running after ${elapsed}s" >&2
