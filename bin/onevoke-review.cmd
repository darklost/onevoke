...
:run_python
"%ONEVOKE_PYTHON_EXE%" -X utf8 "%~dp0onevoke_review.py" %*
exit /b %errorlevel%

:run_py
"%ONEVOKE_PYTHON_EXE%" -3 -X utf8 "%~dp0onevoke_review.py" %*
exit /b %errorlevel%

:python_missing
echo Error: Python 3 is required but py.exe, python.exe, and python3.exe are unavailable. 1>&2
exit /b 127
