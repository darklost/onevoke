...
for /f "delims=" %%P in ('"%SystemRoot%\System32\where.exe" python3.exe 2^>nul') do (
...
:run_python
"%ONEVOKE_PYTHON_EXE%" -X utf8 "%~dp0merge-worktree-memory.py" %*
exit /b %errorlevel%

:run_py
"%ONEVOKE_PYTHON_EXE%" -3 -X utf8 "%~dp0merge-worktree-memory.py" %*
exit /b %errorlevel%

:python_missing
>&2 echo merge-worktree-memory: Python 3 was not found. Install Python 3 or add it to PATH.
exit /b 127
