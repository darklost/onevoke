...
任务 ID 必须匹配 `YYYYMMDD-short-slug-task`; slug 仅使用小写 ASCII 字母、数字和连字符. 用户可见错误信息及规则文档沿用中文和 ASCII 标点. CLI、TUI、Web 与 `install.sh` 的用户可见输出默认中文; 语言优先级为 `--lang` > `config.json` 的 `language` > 环境变量 (`ONEVOKE_LANG`/`LC_ALL`/`LC_MESSAGES`/`LANG` 的 `en` 前缀选英文). `language` 可在 `onevoke welcome` 配置.
