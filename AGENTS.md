...
测试默认针对当前工作树. `tests/test-kanban.py` 可用 `KANBAN_COMMAND` 指向别的入口; 四个 POSIX 审核测试和 `tests/test-windows-review.py` 都用假 Codex/Claude/Grok/Cursor 二进制驱动, 不调用真的 CLI, 也不产生网络请求. Windows 专项测试只在原生 Windows 运行, 其他平台会 skip; POSIX 的 pty/tmux 专项测试不作为 Windows 第一阶段门禁.
