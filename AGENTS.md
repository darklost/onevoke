...
- `bin/onevoke` 提供 `welcome`, `doctor`, `config`, `review`. welcome 只在 tty 中提问, 无 tty 时诊断后正常提示重跑; 依赖安装必须经用户明确选择. MemSearch 不固定版本: CLI 从 PyPI 安装最新版, Codex 插件从官方仓库默认分支获取最新版; 只执行来源匹配且工作树 clean 的源码. `review` 按角色配置分发到 Codex 或 Grok wrapper, 当前任务或项目明确覆盖时可直接调用对应 wrapper.
