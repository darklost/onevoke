...
Codex 的 `project_doc_max_bytes` 默认 32 KiB (`32768` 字节), 全局与项目的 `AGENTS.md` 合计超过就**静默截断**, 不报错. 入口 `ONEVOKE-AGENTS.md` 约 2.7 KiB, 留给项目级的还有约 29.3 KiB — 分册是按需读取的独立文件, 不占这个预算. 不够时在 `~/.codex/config.toml` 调高:
...
选 1 建卡并启动; 选 2 在本会话直接实施; 选 3 继续调整后重问. 纯问答, 只读排查, 纯文档或配置微调, 运维操作及已由 `kanban start` 拉起的会话不提供这组选项.

选 1 后让 Agent 用 `kanban new` 建卡, 并以已确认内容填完模板, 不留 `<填写>`.

```sh
kanban new feature login-retry 登录重试
```

新卡先进入 `backlog`. 默认是单文件小卡; `--large` 是需要独立 spec, 按需计划和完整报告的单卡, 不是任务组. 可独立领取, 验收或终止的工作须拆卡; 多条交付链路组成任务组, 尽量并行. 契约字段和任务组格式见 `kanban rules`.
...
`pick` 校验并执行 `backlog -> todo`; `start` 只接受完整的 `todo` 卡, 原子领取为 `working`, 写负责人和开始时间, 再启动 Agent. 两个命令不传 ID 时只列候选. `start` 使用免确认模式, 应限于可信本机和已确认范围; 模型, launcher, 失败回滚和 TTY 规则见 `kanban rules`.

任务组必须用 tmux. 协调 Agent 转为编排 Agent, 按依赖并行启动就绪卡, 每次巡检间隔不超过 30 分钟, 事件立即处理. 只有全组进入 `done` 才成功; 结束时按卡汇总全部未处理问题, 安全审核超时项标记"超时忽略". 异常退出和交接规则见 `kanban rules`.

### 5. 协调任务并继续规划

启动者协调至任务完成或明确交接; 退出, 阻塞或长期无更新时按 `kanban rules` 恢复. 后台执行期间可讨论下一项需求, 但切换会话前须明确交接协调责任; 任务组编排会话保持到全组结束.

需求会话负责决策, 看板保存契约和状态, 执行 Agent 独立完成任务. 人只在方案, 优先级, 契约或负责人变更, 审核风险和最终验收时介入.
...
实施期只把关键决策, 实际命令, 结果, 环境缺口和 commit 写回任务卡. 不复制会话流水. 大任务按需把计划写进 `plan.md`, 完成后把实际结果写进 `report.md`.
...
看板任务中, `CSA` 或 `Hacker` 经核实成立的必修 finding 从完整问题和选项送达起逐项计时; 15 分钟无决策即记"超时忽略"并继续. 它不算 `PASS` 或接受风险, 须写入任务卡和完成报告. 其余需决策事项不超时.
...
审核豁免按 `REVIEW-RULES.md` 和项目规则判断. 跳过时必须说明触发条件; 用户明确承担风险的跳过还要进入交付说明.

本仓库自身有特例: 纯 Markdown 规则或文档改动不审核; 含脚本、代码或其他非 Markdown 文件时运行 `PM` 和 `QA`, `CSA` 与 `Hacker` 一律 N/A. 安装到其他项目后仍按那些项目的风险和规则判断.
...
实现, 验证记录, 必要审核, 用户验收和适用的代码集成全部完成后, 写小任务完成总结或大任务 `report.md`, 填 `结果: completed`, 再迁移:
...
进入 `done` 后按 `KANBAN-RULES.md` 固定模板汇报. 归档, 删除及报告字段以该规则为准.
...
主流程是 `backlog -> todo -> working -> done -> archived`; 活跃卡也可按规则终止归档, 删除须用户逐项授权. `move` 和 `start` 都以同文件系统原子迁移领取卡片, 只有迁移成功者取得任务. 完整门禁见 `kanban rules`.
