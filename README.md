...
`kanban tui` 在当前终端启动全功能只读看板. 默认按终端宽度显示尽可能多的栏目, 每栏默认最小 40 列 (可用 `-`/`=` 调节并记住), 更窄时少显示或按实际宽度显示单栏, 并保证选中栏可见; `--single` 始终只显示一个栏目. 支持方向键或 `hjkl` 切换栏目和任务, 鼠标点选与滚轮、双击进入详情, 拖选文本或按 `y` 复制到系统剪贴板, PgUp/PgDn 翻页, `/` 搜索 (也可点工具栏), Enter 查看任务卡, `a` 切换存档栏目, `t` 切换 auto/light/dark 主题, `r` 刷新, `q` 退出. 任务卡详情支持 vim 风格翻页 (`Ctrl-d/u`, `Ctrl-f/b`, `gg`/`G`), 正文搜索 (`/`, `n`/`N`), 以及字符/行选择 (`v`/`V`) 后 `y` 复制. 默认每 30 秒自动刷新, 按任务 ID 原位更新并尽量保留选中项和滚动位置.
