...
任务默认建成一个 Markdown 文件. 只有单张卡本身需要独立 `spec.md`、分阶段计划和最终报告时才用 `--large`; 行数多本身不是大任务. 一个总体目标包含多项能独立领取、验收或取消的工作时, 应建成任务组并拆成多张卡, 优先减少依赖以便并行执行. 组内卡片在 `讨论与决策` 填写相同的 `任务组` 标识, 并用 `前置任务` 声明依赖; 同目标、同负责人、同生命周期的内容才留在一张卡内.
...
启动任务组时, 当前协调 Agent 转为编排 Agent. 它先核对全组卡片和无环依赖关系, 将已确认的卡片移进 `todo`, 并行启动所有无前置任务的卡片. 此后只有在一张卡的全部前置卡片进入 `done` 后才启动它; 同时就绪且没有代码或资源冲突的卡片尽量并行.

编排 Agent 不在启动后结束会话. 它每隔 30 分钟运行 `kanban check`, 查看全组状态、执行 Agent、阻塞和完成报告, 及时启动新解锁的卡片或恢复异常退出的执行 Agent. 需要换负责人、改任务契约、放弃卡片或接受风险时由用户决策; 不受影响的并行链路继续运行. 全组卡片都进入 `done` 后, 编排 Agent 汇总组级执行顺序、并行情况、验收结果和遗留项, 才结束编排.

全组完成汇总必须另列“未处理问题”, 按任务卡汇总每张卡报告中的审核未处理项、验证缺口、接受的风险和遗留问题. 每项写任务 ID、来源、档位、影响和未处理理由; 因安全审核等待用户 15 分钟无决策而继续的项标记“超时忽略”.

### 5. 协调任务并继续规划

启动者在任务完成或明确交接前持续负责协调. tmux 模式下定期查看卡片状态和执行 Agent 是否仍在运行; foreground 模式等待 Agent 退出后检查任务结果. 发现退出、阻塞或状态长期未更新时, 核实卡片和 worktree 现场并按异常恢复规则处理. 任务组的编排 Agent 还须按上一节保持会话, 每 30 分钟巡检并启动新解锁的卡片.

单卡在后台执行时, 协调会话可继续讨论下一项需求, 但不能因创建新会话而丢失已启动任务的跟踪责任. 需要切换上下文时, 先把协调责任明确交给另一个持续运行的 Agent. 任务组的编排会话在全组完成前保持运行, 不用 `/new` 替换.
...
- 人只在方案确认、任务优先级、需要改变契约或负责人、审核风险和最终验收处介入.
...
`CSA` 或 `Hacker` 的 finding 需要用户决策时, Agent 发送完整问题、影响、修复方法和选项后开始计时. 15 分钟没有用户决策, 该 finding 记为“超时忽略”并继续进入 `QA`; 它不算 `PASS` 或用户接受风险. Agent 立即把问题和时间写回任务卡, 并在完成报告的“审核未处理项”逐项指出. `PM`/`QA` 必修 finding 和其他用户确认不适用这个超时规则.
