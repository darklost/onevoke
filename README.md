...
任务默认建成单文件小卡. `--large` 表示一张需要独立 spec、计划和报告的复杂卡, 不是任务组. 能独立领取、验收或终止的工作必须拆卡; 一个总体目标含多条交付链路时建任务组, 尽量减少依赖并行执行. 组内卡片在 `讨论与决策` 记录同一任务组 ID 和前置任务, 具体格式见 `kanban rules`.
...
任务组必须用 tmux 启动. 当前协调 Agent 转为编排 Agent, 校验依赖后并行启动所有就绪卡; 只有全部前置卡进入 `done` 才能解锁后继卡. 编排会话保持运行, 相邻巡检不超过 30 分钟, 发现完成或消息时立即处理. `working` 卡的执行 Agent 退出后保留现场, 不再次调用 `kanban start`; 是否交接、改契约或终止由用户决定.

全部卡片进入 `done` 才算任务组成功. 编排结束时汇总组级结果, 并按任务卡列出所有审核未处理项、验证缺口、接受的风险和遗留问题; 安全审核超时项标记“超时忽略”.

### 5. 协调任务并继续规划

启动者在任务完成或明确交接前持续负责协调. tmux 模式定期查看卡片和执行 Agent; foreground 模式等待退出后检查结果. 发现退出、阻塞或长期无更新时, 核实卡片和 worktree 后按异常恢复规则处理.
...
`CSA` 或 `Hacker` 经核实成立的必修 finding 逐项提交用户决策. 每项从完整问题和选项送达起独立计时; 15 分钟无明确决策就记为 "超时忽略" 并继续 `QA`. 它不算 `PASS` 或用户接受风险, 必须写入任务卡和完成报告. `PM`/`QA` finding 及其他用户确认不适用该超时.
