...
任务默认建成一个 Markdown 文件. 只有单张卡本身需要独立 `spec.md`、分阶段计划和最终报告时才用 `--large`; 行数多本身不是大任务. 一个总体目标包含多项能独立领取、验收或取消的工作时, 应建成任务组并拆成多张卡, 优先减少依赖以便并行执行. 组内卡片填写相同的 `任务组` 标识, 并用 `前置任务` 声明依赖; 同目标、同负责人、同生命周期的内容才留在一张卡内.
